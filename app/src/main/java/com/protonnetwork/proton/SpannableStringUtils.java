...
public class SpannableStringUtils {
	...
	public static class Builder {
		...
		private int roundRectHorizontalSpace;
      ...
      /*  *//**
		 ...
		 * @return
		 */
		public Builder setRoundRectBackground(int color, int radius,int textColor,int textSize,int roundRectHorizontalSpace) {
			this.roundRectBackgroundColor = color;
			this.roundRectRadius = radius;
			this.roundRectTextColor = textColor;
			this.roundRectTextSize = textSize;
			this.roundRectHorizontalSpace = roundRectHorizontalSpace;
		...
		private void setSpan() {
			...
			if(isNormal){
				mBuilder.setSpan(new StyleSpan(Typeface.NORMAL), start, end, flag);
				isNormal = false;
			}
			if (isBold) {
           ...
           /* if (align != null) {
			...
			if (imageIsBitmap || imageIsDrawable || imageIsUri || imageIsResourceId) {
				if (imageIsBitmap) {
					mBuilder.setSpan(new ImageSpan(ProtonApplication.getAppContext(), bitmap), start, end, flag);
					bitmap = null;
					imageIsBitmap = false;
					...
					mBuilder.setSpan(new ImageSpan(ProtonApplication.getAppContext(), uri), start, end, flag);
					uri = null;
					imageIsUri = false;
				} else {
					mBuilder.setSpan(new ImageSpan(ProtonApplication.getAppContext(), resourceId), start, end, flag);
					resourceId = 0;
			...
			if(isRoundRectBackground){
				mBuilder.setSpan(new RoundRectBackgroundSpan(roundRectBackgroundColor, roundRectRadius, roundRectTextColor,
															 roundRectTextSize, roundRectHorizontalSpace), start, end, flag);
				isRoundRectBackground = false;
			}
            /*if (isBlur) {
